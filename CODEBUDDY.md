...
- **待拍板项必须按《AI协作铁律与踩坑复盘》§一 · G 的「四要素」写**（谁 / 干什么的 / 建议 / 影响范围）—— 否则七叔看不懂在拍什么。

**③ 换窗口判据**：**不只在"里程碑闭环"时换** —— 读完一个大文件 ＋ 做完一批改动，即可换窗（接续成本远低于继续拖着）。换窗前必须：更新交接说明 ＋ 提交 ＋ 工作树干净。

**④ 容量上限（可机械检查）**：`CODEBUDDY.md` **≤ 12 KB** ｜ `过程产出/交接说明*.md` **≤ 5 KB** ｜ `技术决策/AI协作铁律与踩坑复盘.md` **≤ 25 KB**（查：`git cat-file -s HEAD:<path>`）。**到顶＝先压缩老内容，再加新内容。**
> ⚠ 其中**铁律＝每轮重放层**（动工前必读全文）：它涨 1 KB 的代价 ≫ 状态类文档涨 1 KB（2026-09-15 实测：常驻层瘦身省下的 9 KB，被铁律涨的 5 KB 抵消掉一半以上，净收益只剩 ~10%）。
