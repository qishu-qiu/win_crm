...
| 内容 | 唯一落点 |
| --- | --- |
| **AI 行为规则（★ 每轮必读，≤3 KB）** | `技术决策/AI执行清单.md` |
| 踩坑案例库 / 为什么这么定（**按需查，不重放**） | `技术决策/AI协作铁律与踩坑复盘.md` |
| 当前进度 / 证据 / 下一步 | `过程产出/交接说明.md`（★ **固定名**，不随里程碑改；历史查 `git log`） |
| **欠账（做不了 / 只做一半 / 待回来补）** | `过程产出/欠账登记表.md` |
...
**开工前先读《AI执行清单》**（每轮必读 · ≤3 KB · 只写"时机 → 动作"）；遇到具体坑再按需翻《AI协作铁律与踩坑复盘》（案例库）；新窗口先读交接说明与《欠账登记表》。
...
- ⚠ **端口**：后端 **3000**，前端 dev **5173**（均已在配置里写死)；vite 已设 `server.host: true` → `127.0.0.1` 与 `localhost` **均可**访问。
...
- ⚠ **括号里的列举是「举例」，不是「穷尽清单」**：规格中「（登录、改手机号…）」这类表述**默认非穷尽**，只有写明「完整清单」「一律」或 §十六 表头那种"表里没有才代表允许"才算穷尽。**不许**由"清单外"推出「不做」，**也不许**推出「可以做」—— 两种误读都翻过车（本项目已 5 次，→ 需求 §1.3）。
...
> **为什么有这节**：单次会话的输入，绝大部分**不是"读规则"花的，而是"读进来的东西被之后每一轮反复重放"花的**。省 token 的正解＝**少重复读 / 早换窗 / 砍索引留规则** —— **不是"少读规则"**。

**① 读取纪律**
- **同一会话内，同一个文件只全文读一次**；同一文件二次改动前**只读改动点前后约 20 行**，不重读全文。
- **先 `grep -n` 定位行号，再定向读片段**；规格类按 §节 读，不整篇读。
- **改动前必须 `grep` 全部引用**（删 / 改一节会断掉别处的指针 —— **不可省**）。
- 子代理任务要**限死返回长度**（如"每条 ≤ 2 行、总长 ≤ 3,000 字"）。

**② 输出纪律**
- git 一律用 `--stat` / `--short` / `--numstat`，**永不跑全量 diff**。
- 统计类命令（体积 / 行数）**只在整个任务收尾时跑一次**，不中途反复跑。
- 校验类 `grep` **合并成一条**（多 pattern 一次跑）。
- **待拍板项必须按《AI执行清单》#21「四要素」写**（谁 / 干什么的 / 建议 / 影响范围）—— 否则七叔看不懂在拍什么。

**③ 换窗口判据（★ 2026-09-16 强化）**：**不只在"里程碑闭环"时换** —— **本轮完成 ≥2 次提交，或全文读过 ≥2 个大文件，就该换**，且 **AI 必须主动提示**（2026-09-16 实测：8 个提交挤一个窗，成本按轮数**平方**涨）。换窗前必须：更新交接说明 ＋ 提交 ＋ 工作树干净。

**④ 容量上限（可机械检查）**：`CODEBUDDY.md` **≤ 12 KB** ｜ `过程产出/交接说明*.md` **≤ 5 KB** ｜ **`技术决策/AI执行清单.md` ≤ 3 KB**（查：`git cat-file -s HEAD:<path>`）。**到顶＝先压缩老内容，再加新内容。**
> ⚠ **《AI执行清单》＝唯一的每轮重放层**（开工必读全文）：它涨 1 KB 的代价 ≫ 状态类文档涨 1 KB（2026-09-15 实测：常驻层瘦身省下的 9 KB，被重放层涨的 5 KB 抵消掉一半以上，净收益只剩 ~10%）。
> ★ `技术决策/AI协作铁律与踩坑复盘.md`（**案例库**）**不设上限** —— 它**按需查、不重放**（2026-09-16 拆分：每轮重放层从 21 KB 降到 ≤3 KB）。

**⑤ 校验不可省（省了必漂移）**：① 改前引用排查 ② 改后校验（损坏签名 / 失效引用 / 旧值残留）③ 唯一落点确认 ④ 提交前逐文件认领（禁 `git add -A`）。
> **可省的是「导航 / 状态 / 索引 / 证据」；绝不能省的是「规则正文」与上述四项校验。**
