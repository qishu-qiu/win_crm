...
> 本项目是**文档驱动（spec-driven）**的销售 CRM 系统。当前处于「**M3 已闭环（A 域登录 ＋ B 域建档/查重 ＋ C 域业务关系·私海·公海，均含前端页），下一步 M4（D 域：跟单事件 / 承诺 / 工作台）**」阶段：**真相源是文档，不是代码**。动手前先按本文与 `README.md` 的规则锁定口径。

> **★ 动工前必读（AI 行为约束）**：`技术决策/AI协作铁律与踩坑复盘.md`（现行 **V1.8**）—— **AI 协作元规则的唯一落点**（AI 行为硬约束 / 踩坑复盘 / 通例）。**凡写文件 / 装依赖 / 跑命令 / 调用有副作用工具前，先读它**；今后 WorkBuddy 与 CodeBuddy 新增同类规则一律追加到该文档。
>
> **★ 下个窗口开工前**：先读 `过程产出/交接说明-M1（2026-09-14 窗口）.md` —— 「状态 / 证据 / 缺口 / 踩坑」的唯一落点（本文不复述）。

## 一、当前状态（务必先读）

- **后端 `服务端/src/` 已成形**（NestJS 12 ＋ Prisma 7.10.0）：`main.ts` / `worker.ts` / `app.module.ts`、`kernel/`（context / events / audit / errors / common）、`shared/`（guards / interceptors / filters / pipes）、`modules/org/`（A 域四层，**M1 全绿**）、`modules/company/`（B 域四层，**M2 全绿**）、`modules/relation/`（C 域四层，**M3 全绿**）。已跑通：登录（**手机号 / 账号名双通道**）、刷新、`/account/me`、`/org/departments|employees|roles|permissions`（**`/org/employees` 按 G7 收敛**）、`GET/POST /companies`、`POST /companies/search-dup`、`GET/POST /contacts`、`GET /companies/:id/contacts`、`GET/POST /relations`（**私海 / 公海 ＋ 数据范围收敛**）、`GET/PUT /relations/:id`、`GET/POST /relations/:id/members`。
...
- **前端 `前端/` 已有真实页面**：`views/LoginView.vue`（一个输入框＝手机号或账号名）＋ `views/WorkbenchView.vue`（首屏空壳）＋ `views/EntryView.vue`（**M2-17 建档页**：查重 → 建档 / 用现有 → 建联系人）＋ `views/RelationListView.vue`（**M3-14 业务关系列表**：私海 / 公海页签，**只读**）；`api/types.ts` 由 `gen:types` 生成；**未引 vue-router / pinia**（顶栏 ＋ 视图切换在 `App.vue`）。
...
| 层 | 文档（位置） | 管什么 |
| --- | --- | --- |
| ① | 《销售CRM业务需求文档》V1.26（需求规格/） | 业务规则 / 流程 / 权限口径 |
| ② | 《销售CRM数据架构文档》V1.31（需求规格/） | 表 / 字段 / 索引 / 字典 / 数据权限 |
| ③ | 《销售CRM接口API文档》V1.16（需求规格/） | 接口入参/出参 / 错误码 |
...
已定案：**竖切一条完整动线** —— 登录 → 建公司/联系人 → 建业务关系 → 写一条跟单 → 工作台看到它（穿透 A/B/C/D 四域）。8 步：**0** ✅ 打地基（kernel + Prisma + OpenAPI + ESLint 边界）→ **1** ✅ A 域登录（含 `/account/me`、四个组织只读接口、登录页）→ **2** ✅ B 域建档/查重（含前端建档页；字典已按数据架构 §十三落库）→ **3** ✅ C 域建关系/私海（含前端列表页）→ **4** ⏳ D 域写跟单/时间线/工作台（**下一步**）→ **5** 横切层（数据范围+脱敏+审计）→ **6** 接前端（`gen:types`）→ **7** Worker 骨架（掉海预警先只告警）→ **8** 验收。每步＝一次可提交的还原点（架构说明 §九）。
