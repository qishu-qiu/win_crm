...
| 当前进度 / 证据 / 缺口 / 下一步 | `过程产出/交接说明-M5（2026-09-15 窗口）.md` |
...
**④ 容量上限（可机械检查）**：`CODEBUDDY.md` **≤ 12 KB** ｜ `过程产出/交接说明*.md` **≤ 5 KB** ｜ `技术决策/AI协作铁律与踩坑复盘.md` **≤ 25 KB** ｜ `.workbuddy/memory/MEMORY.md` **≤ 8 KB**。前三份查 `git cat-file -s HEAD:<path>`；⚠ **MEMORY 不在 git**（`.workbuddy/` 被 `.gitignore` 忽略）→ 只能用文件大小 `(Get-Item <path>).Length`，**且它没有沿革、误改不可恢复**。**到顶＝先压缩老内容，再加新内容。**
> ⚠ 后两份是**每轮重放**层（铁律＝动工前必读；MEMORY＝长期记忆）：它们涨 1 KB 的代价 ≫ 状态类文档涨 1 KB（2026-09-15 实测：常驻层瘦身省下的 9 KB，被铁律涨的 5 KB 抵消掉一半以上，净收益只剩 ~10%）。
