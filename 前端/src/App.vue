        ...
        <header class="shell-header">
          ...
          <!--
            ...
            ★ **`#overlay` 槽里的第一个节点，必须是 `<a-menu>` 本身**：AntD 取
              ...
              弹层里就只剩一个**空注释节点**（`innerHTML` 里除注释什么都没有、**高 0**）——
              表现是"点了 / 悬停都毫无反应"，而 **lint 与 build 全绿**（生产构建会剥掉注释、
              反而正常，故只有真跑 dev 页面才暴露）。2026-09-20 实测踩中。
            ⚠ **注释正文里绝不能出现"注释结束符"那三个字符**（2026-09-20 当场又踩一次：写这段
              注释时把那三个字符打了出来 ⇒ 注释**提前闭合**，后半段被当成**正文**渲染到了顶栏上，
              而 **lint / build 依然全绿**）。⇒ 要提它就写"注释结束符"这几个字，**别把字符打出来**。
