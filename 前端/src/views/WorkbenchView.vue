 ...
 *   `homeNameOf(role)` 是**角色首屏名**（§4.1 主入口列，答的是「登录后**该去哪个页面**」），
 *   本页不拿它当标题 —— 否则经理登录会在**装着今日动线的页面上**写着「数据看板」
 *   （页面名与内容两套说法）。
 *   ★ 2026-09-18 侧栏化时同一口径落到菜单：菜单项也一律取**页面名**（`access.ts` 的 `NAV_GROUPS`），
 *     不再拿首屏名当菜单项名 —— 否则经理看到「数据看板」、点进去却是工作台（→ 欠账 D-36）。
 */
