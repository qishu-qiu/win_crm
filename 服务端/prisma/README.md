...
> **真相源**：《需求规格/销售CRM数据架构文档》（§三~§九 表、§十 索引、§十五 落库口径）。本文只记「Prisma 与 MySQL 层面的落法」，不重复业务规则。
...
| 项 | 内容 |
| --- | --- |
| `prisma/schema.prisma` | **47 张业务表**，与数据架构一致（第 47 张＝`idempotency_key`，2026-09-20 `0010`，→ 接口 §2.5） |
...
- **47 张表**，真相源＝数据架构文档（第 47 张 `idempotency_key` 见 §A15 / migration `0010`）。
...
| 8 | **构建链路（尤其 `npm ci --omit=dev`）** | `package.json` 已含 `prisma:generate` / `build` / `postinstall`，本机 `npm run build` 已实测通过（⚠ **2026-09-20**：本工作区的**环境删除守卫**会拦在 `nest build` 的"清 dist"这一步 ⇒ 已把 `nest-cli.json` 的 `deleteOutDir` 关掉、构建只覆盖不删；**要绝对干净的产物**时跑 `npm run clean`（弹一次批量删除确认，人工点），→《欠账登记表》**D-58**） | `npm ci` 会自动跑 `postinstall → prisma generate`；⚠ 但 **`--omit=dev` 时 `prisma` CLI 不在 → `postinstall` 会失败**：改用**多阶段构建**（构建阶段装 devDeps 跑 `npm run build`，运行阶段只带 `dist/` ＋ prod deps ＋ 已生成的 client），或 `npm ci --ignore-scripts` 后自行 `npm run prisma:generate`。**首次部署必跑 `npm run build` 验证** |
