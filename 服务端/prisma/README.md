...
> **真相源**：《需求规格/销售CRM数据架构文档》**V1.31**（§三~§九 表、§十 索引、§十五 落库口径）。本文只记「Prisma 与 MySQL 层面的落法」，不重复业务规则。
...
| 项 | 内容 |
| --- | --- |
| `prisma/schema.prisma` | **46 张业务表**，与数据架构 V1.31 一致 |
...
三份 migration **均已在真库 `win_crm`（本机 phpStudy MySQL 8.0.12）执行**。
...
> **⚠ 铁律（贯穿全部历史）**：`0001_init` / `0002` / `0003` **均已登记，一律不得改动**（改文件会与 `_prisma_migrations` 的**校验和不一致**）。**注释 / 结构修正一律走「新增量 migration」**。
...
`0001_init` → `0002_company_capital_legal_person` → `0003_username_and_phone_lock`，**并分别** `npx prisma migrate resolve --applied <name>`。
...
**002 字典种子的两个 ⚠ 待裁项（别当既定口径）**：

1. **`workflow_stage` 码**：种子用 **`1`~`7`**（＝ `business_relation.stage` 数值列口径 ＋ 数据架构 §十三 编号写法），而《接口API文档》V1.16 §2.6 的 `stage` 举例是**英文码**（`first_contact … churned`）—— **两套并存，需裁一次**。
...
| 6 | **migration 全量重放** | 已应用 `0001_init` ＋ `0002` ＋ `0003` | 空库上按序跑三份 → 逐份 `migrate resolve --applied`（见 §4.4），再 `migrate status` 断言 **`up to date!`** |
