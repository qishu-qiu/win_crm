...
//
// 口径来源（★ 真相源，勿自造）：
//   · 《销售CRM接口API文档》V1.12 §2.3 统一响应包：`{ code, message, request_id, data }`
//     —— `code = 0` 成功、非 0 = 业务错误；`message` 是**给销售看的人话**。
//   · 同 §2.4 错误码表：HTTP 状态与 `code` **各成一套**（如 409 ↔ 20004 / 204xx），不可混用。
//   · 《销售CRM架构设计说明》V1.1 §7.5 异常映射：P2002 → 409，**不把 DB 原话丢给销售**。
//
// 分层约束（架构 §5.4）：kernel 零业务、谁都能用 —— 本文件**不许 import `modules/*`**。
// =============================================================================

/**
 * 错误码表（→《接口API文档》V1.12 §2.4，**逐行转写**；新增码必须先在规格里出现，不得自造）
 */
