...
常用业务码（204xx）：`20401` 撞单已存在｜`20402` 超过上限（标签/特质）｜`20403` 必填未填（死因 / 开发价值 / 签约校验清单 / **动线 `ignored` 原因 / 承诺豁免原因**）｜`20404` 预约未完成禁止｜`20405` 解锁申请已存在｜`20406` 关系已激活｜**`20407` 跨部门 @求助**（@求助仅限同部门，跨部门须走正式协同审批 —— **2026-09-15 补**：原规则只写了要求、没给码；按本表「422 ＋ 204xx ＝ 业务校验不通过」归类）｜**`20408` 公海关系未领取禁止写入**（**2026-09-18 补**：无主关系**只可改「开发价值」`value_tier`**，写跟单 / 快速标记 / 建改承诺 / 换阶段 / 加删成员一律拒 —— 同「422 ＋ 204xx」归类，与 `20404` 同为"状态不允许"一族）｜**`20409` 部门未承接该产品线**（**2026-09-22 补**：「公司 × 部门 × 产品线」里的**部门 × 产品线**必须是**配置里存在的承接组合**（A7 `dept_ids`），否则 422 —— 判据与录入页产品线下拉**同源**，→ 架构 §7.2「可建产品线范围」/《欠账登记表》**D-74**）。
...
- `GET /relations`：**列表类一律分页**（→ §2.3 / §2.7）—— 入参 `tab` ＋ `page`（默认 1）/ `page_size`（默认 20、最大 100），出参＝ **`{list,total,page,page_size}`**（**不是裸数组**）。**排序 / 搜索（V1.33 · D-07）**：`order_by`（**白名单**：`id` 默认 / `created_at` / `last_event_at` / `stage`；**白名单外 → 400**）＋ `desc`（缺省 `true`）＋ `keyword`（**按公司名**模糊搜；索引前提见 §2.7 补记）。**筛选（V1.21）**：`view`（视图：`all` 我的全部 / `following` 跟进中＝阶段 1~5 / `cooperated` 已合作＝阶段 6 / `churned` 已流失＝阶段 7）＋ `urgency`（紧迫档**多选**、逗号分隔）——**由后端过滤**（前端本地过滤在分页后只能筛当前页），`total` 数的是**筛完之后**的总数。`list` 项含 `stage`（彩色点）、`urgency`、`value_tier`、逾期标红、**`drop_in_x_days`**（**距掉海还剩几个自然日**，按自然日 Asia/Shanghai；`0`＝**到期当天** ⇒ 列表标红「**到期当天掉公海 ⚠**」、**负数**＝到期日已过 ⇒ 次日掉落、`null`＝**判不了**；语义见 §5.6）、竞争徽标、跨线 `amount_masked`。**★ 落点（V1.39）**：本端点由 C 域迁至**聚合层** `relation-aggregate` —— `drop_in_x_days` 要 F 域的公海规则才算得出，而 **C(L3) 读不到 F(L4)**（架构 §3），跨域装配只能发生在更高编排层（→《欠账登记表》D-10 / D-61 桥③）。⚠ 原句括号里写的「24h 掉公海⚠」是**按小时口径的残留**，随《废止口径登记表》**#41** 作废（口径＝**最小单位是天**，→ 需求 §6.3）。
...
- `GET /account/me` → `UserVO` ＝ `{id,name,username?,role,dept:{id,name},managed_dept_ids:[],activatable_dept_ids:[],product_line_ids:[],permissions:{"perm_key":"level"},theme,nav_open:[]}`
  ...
  - **`product_line_ids`**（同 V1.40）：**本人关联的产品线集合**（→ 员工 A7 `product_line_ids`）。⚠ **2026-09-22 起前端已不消费本键**：录入页 / 联系人详情的**产品线下拉**改按「**所选部门承接的产品线**」收敛（`product_line.dept_ids`，→ 架构 §7.2「可建产品线范围」/《欠账登记表》**D-74**）—— 原按本键过滤是**假限制**（把可选线缩到"我挂的"，比口径窄）。服务端**同批**落地同一判据：不是承接组合 → **422 / `20409`**（→ §5.6）。**本键的存废待定**（当前无消费方）。
...
- **列表项** `{id,company:{id,name},dept:{id,name},product_line:{id,name,color_key},stage:1-7,urgency,value_tier,customer_level,owner:{id,name},last_event_at,drop_in_x_days,overdue,competition,amount|amount_masked,old_customer,is_weekly}`
  - **`drop_in_x_days`** ＝ **距掉海还剩几个自然日**（（**派生字段、库不存**，→ §5.1 派生字段清单））：`0` ＝ **今天到期**（列表按「到期当天掉公海 ⚠」标红）、**负数** ＝ 到期日已过（**次日掉落**）、正数 ＝ 还有几天。**判不了就给 `null`** —— 公海（无主）/ 没有规则命中 / 规则没配 `follow_freq_days` ⇒ **一律不编 `0`**（编了会把"算不出"显示成"今天到期"）。天数口径（按自然日、三档）唯一落点＝**需求 §6.3**（→《废止口径登记表》#41）。
  - ⚠ 本字段**只有列表项有**：详情 `GET /relations/:id` 本期**不返回**它（→《欠账登记表》D-10 残余）；ⓘ 它是本文件里**第一个由聚合层拼装**的关系字段（C 域给锚点、F 域算天数，装配在 `relation-aggregate`，→ D-61 桥③）。
...
- `POST /relations`（激活）req `{company_id,dept_id,product_line_id}`（撞 `uk_active_rel` → **409/20401**，**人话分档**：同三元组已有**私海** → 「已有归属，请走转交或协同」；已有**公海**行 → 「该客户在本部门·产品线已在公海，请**直接领取**」—— 公海行走 `POST /sea/company/:id/claim` 承接**同一条**，**不另建行**，→ 需求 §6.3 /《欠账登记表》D-53）。**校验顺序与错误（V1.41）**：① `dept_id` 越权 / 只读角色 → **403 / `20003`**；② 三个 id 有不存在者 → **400**；③ **「部门 × 产品线」不是承接组合**（`product_line.dept_ids` 不含该部门）→ **422 / `20409`**；④ 撞活跃唯一键 → **409 / 20401**（人话分档见上）。⚠ ③ **不报 400**：两个 id 都真、也都在可建范围内，**错的是组合**（换一条线即可重试），与「id 指错」是两回事。
- `POST /contacts/:id/activate-relation`（**V1.23 新增**，2026-09-16 拍板）：把一条「**待关联**」联系人（还没挂公司）**关联到公司 ＋ 激活一条业务关系**，并把该联系人名下**未挂关系**的跟单**批量挂到新关系**（历史不断，→ 需求 §6.1 ④）。
  ...
  - **错误**：三元组已有活跃关系 → **409 / 20401**（与 `POST /relations` **同一句人话**）；**该联系人已挂过公司**（不是「待关联」）→ **409**（防重复触发）；部门越权 / 只读角色 → **403**；各类 id 不存在 → **400**；**「部门 × 产品线」不是承接组合 → 422 / `20409`**（与 `POST /relations` **共用同一道校验** —— D 域回头调的是同一个 `createRelation`，不会一边收一边漏）。
