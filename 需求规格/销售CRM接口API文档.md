...
| **V1.9** | **2026-09-11** | **口径残留校正（L0 联动 · ★ 无端点/无 DTO 变更）**：§2.4 错误码映射说明校正——原文"绝不直接透 MySQL `1062`"易被 AI 误读为"应 catch 1062"；**统一为 Prisma 口径**：唯一冲突一律映射 **409/422**，**DB 层 MySQL `1062` 与 Prisma 层 `P2002` 都只在服务端内部消化**（`meta.target` 取约束名回友好提示）。配套真相源同步（需求 V1.20 / 数据架构 V1.24 / 前端 V1.12 / 设计规范 V1.0）。 |
