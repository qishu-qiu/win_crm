...
> 配套真相源：《销售CRM业务需求文档》**V1.28**、《销售CRM数据架构文档》**V1.33**、《销售CRM设计规范》V1.0、《销售CRM前端页面与交互文档》**V1.16**（均 需求规格/）。
> **版本沿革**：文档内不留「修改记录」章节（2026-09-12 决定 →《废止口径登记表》#22），沿革查 `git log --follow -- 需求规格/销售CRM接口API文档.md`。
> 生效日期：2026-09-15 ｜ 状态：**V1.18**（**承诺收尾三态补齐**：`done` 兑现 / `cancelled` 取消（**录错了 / 不成立**，不填原因）/ `waived` 豁免（**确有其事但做不成，必填原因** `waive_reason`）—— 承诺出参加 `waive_reason`、`PUT` 入参形状补全（→ 需求 §10.1 / 数据架构 D1 V1.32）。沿用：阶段 `stage` 为数字 `1`~`7`（英文码作废 →《废止口径登记表》#37）；成功响应一律 200；`20407`＝跨部门 @求助；联系方式默认全可见 ＋ 联系人可上锁；登录＝手机号 / 账号名双通道 · 紧随需求 V1.28 / 数据架构 V1.33 / 前端 V1.16）。
