...
- 详情 `{...company字段, completeness:{c1,c2,c3}, profile_tags:{identity:[{tag_id,tag_code,label}],policy:[],decision_chain:{tag_id,label}?}, contacts:[ContactBrief], relations_summary:[{dept,product_line:{id,name,color_key},sign_date,amount|amount_masked}], event_count_30d}`
  ...
  - **★ `relations_summary` 本期子集（D-61 · 2026-09-21）**：数组项**仅含 `dept` / `product_line`**（来自 C 域 `business_relation` 按公司分组的真实业务线）；`sign_date` / `amount` 来自 **E 域 `contract`**，本期 E 域未建 module ⇒ **暂不出、不编假值**，待 E 域就绪由聚合层补（→ D-61 后续）。
  - **★ `event_count_30d` 待补（D-61 后续）**：近 30 天跟单计数来自 **D 域 `action_event`**，本期 `engine` 尚无「按 company 聚合近 30 天跟单」出口 ⇒ **整键暂不出、不编假值**，待 D 域建出口后由聚合层补。
