# 销售 CRM 接口 API 文档 V1.28

> **⚠ 开工前必读**：先读《**废止口径登记表**》（需求规格/）——已废止的旧说法不得作为实现依据。
> 文档性质：四件套之三（①业务需求 ②数据架构 ③**接口 API** ④前端页面与交互）。
> 配套真相源：《销售CRM业务需求文档》**V1.34**、《销售CRM数据架构文档》**V1.37**、《销售CRM设计规范》V1.2、《销售CRM前端页面与交互文档》**V1.21**（均 需求规格/）。
> **版本沿革**：文档内不留「修改记录」章节（2026-09-12 决定 →《废止口径登记表》#22），沿革查 `git log --follow -- 需求规格/销售CRM接口API文档.md`。
> 生效日期：2026-09-21 ｜ 状态：**V1.34**（**D-61 桥③ 续：公司详情 `event_count_30d` 落地**（2026-09-21 ·《欠账登记表》**D-61**）：`GET /companies/:id` 的**近 30 天跟单计数** —— 口径＝**近 30 个自然日**该公司各关系下的跟单条数合计（**派生、不落列**）、**只计当前查看者可见的关系**（§2.2 私海四档）、**不计系统事件**（`action_type=system`：建档 / 领取，→ D2）；实现＝聚合层拼装 → **D 域出口**（按关系集计数）＋ **C 域可见性出口**（哪些关系可见），**B / C / D 三域零改动**（桥③ 的长期价值兑现）。⚠ 同一家公司**不同角色看到的数字可以不同**。同批：`CompanyDetailVoDto.completeness` 由**内联字面量**改为**具名 DTO** —— 原先 Swagger 认不出内联对象，`gen:types` 生成的是 **`Record<string, never>`**（前端拿到等于空对象）。**沿用 V1.33**（**§2.7 通用约定铺开：C 域排序 / 关键词 ＋ B 域分页**（2026-09-20 ·《欠账登记表》**D-07 / D-08**）：① **D-07** —— `GET /relations` 新增 `order_by`（**白名单**：`id` 默认 / `created_at` / `last_event_at` / `stage`；**白名单外 → 400**）＋ `desc`（缺省 `true`）＋ `keyword`（**按公司名**模糊搜），§2.7 补记**铺开进度 / 白名单口径 / 索引缺口 / `start_at`·`end_at` 待拍**四条；② **D-08** —— B 域三个列表由裸数组改为 §2.3 分页形态（`GET /companies` / `GET /contacts` / `GET /companies/:id/contacts`）。**沿用 V1.32**（**B 域三个列表铺开分页**（2026-09-20 ·《欠账登记表》**D-08**）：`GET /companies` / `GET /contacts` / `GET /companies/:id/contacts` 由**裸数组**改为 §2.3 分页形态 **`{list,total,page,page_size}`**（入参 `page` / `page_size`）；原「最近 100 条截断」作废；排序：公司 / 联系人列表 `id desc`、公司联系人 `is_current desc, id desc`（在职在前）。⚠ §2.7 的 `order_by` / `keyword` / 时间范围**仍未铺到 B 域**（→ D-07）。**沿用 V1.31**（**幂等头 `Idempotency-Key` 口径补齐并落地**（2026-09-20 拍板 ·《欠账登记表》D-06）：§2.5 由原来**两句**扩为**十一条口径** —— 头部格式与长度上限（≤64 字符，超长 → 400）、作用域 ＝ **登录人 × 端点 × key**（端点取**路由模板**非实际路径）、**TTL 24h**、**不带头的写请求放行**（★ 过渡期口径，前端接完作废）、**同 key 异入参 → 409 / `20004`**（指纹 ＝ `SHA-256(body)`）、重放响应形状与首次一致、**幂等键重复不算限流**、**不防「并发同 key」双执行**（竞态双写仍靠条件 UPDATE）。数据落点＝**新增 `idempotency_key` 表**（→《数据架构》§A15；表数 46 → **47**）。**沿用 V1.30**（**性别值域落定**（2026-09-20 拍板 ·《欠账登记表》D-49②）：`contact.gender` 的**唯一值域 ＝《数据架构》B2 的 `male` / `female` / `unknown`** —— 本文件原先**只有一个词 `gender`、从未给值域**（建档只校验「1~8 位字符串」）⇒ §5.5 加指针、建档 / 改收紧为枚举（非法值 → **400 / 20001**）。★ `unknown` ＝「**明确不便说**」，与「**从未填过**」（不传本字段 ＝ `NULL`）**不是一回事**。**沿用 V1.29**（**备用号号型值域收敛**（2026-09-20 拍板 ·《欠账登记表》D-49①）：`extra_phones[].type` 的**唯一值域 ＝《数据架构》B2 的 `mobile` / `tel` / `wechat`** —— 本文件原先只在 §5.5 沿用出参 DTO 声明的 `landline` / `other`（**同一字段两套值域**）⇒ 本版**作废那两码**，§5.5 值域改为**指针**（→《废止口径登记表》#39）；旧码仅作**历史数据兜底**，新写入口不再产生。**沿用 V1.28**（**三处契约缺口回填**（2026-09-20 · D-51 / D-16 / D-24）：① §5.7 `POST /contacts/:id/events` 判定口径 —— 谁能写（**当前归属人**）/ 已挂公司同样 403 / `contact_id` 与路径冲突 → 400 / 落库 `relation_id` 与 `owner_snapshot` 均 `null`；② §5.7 `PUT /relations/:id/commitments` 入参形状 —— `{id,status?,waive_reason?,due_at?,remind_at?}`，`id` **由 body 指认目标**；③ §5.7 `POST /events/quick-mark` 出参 `{marked}`。三者都是**补齐既有实现落地的规格缺口**，**不是新口径** —— 依据＝需求 §6.1 ⑦⑨ / §10.1 / §10.2 ＋ 同族端点，不由实现反过来定义契约。**沿用 V1.27**（**公海「领取到私海」端点形状回填**（2026-09-20 · D-33 · F-01）：`POST /sea/company/:id/claim` 的 **req / resp / 错误码已回填 §5.16**（`:id` ＝**公司 id**；req `{dept_id, product_line_id}`；resp ＝ §5.6 列表项 ＋ `claimed_at`），§4.5 那行同批补「**定位规则**」＋「**谁能领**」两句 —— 实现落点 `服务端/src/modules/sea/`（片 1 `5c59d9f` / 片 2 `7f24d8f`，真库真机已验）。**沿用 V1.26**：**个人偏好落库 ＋ 端点落地**（2026-09-18 · D-37 / D-41①）：数据架构 `employee` 加 `theme` / `nav_open`（migration `0009`）；`PUT /account/preferences` **已实现** —— 入参分 `theme` / `nav_open` **两键**、**部分更新**（不给的键原样保留，一个都不给也回 200 且**不改库**）、出参＝**更新后的 `UserVO`**；`GET /account/me` 的 `UserVO` 同批加这两个字段（→ §4.14.9 / §5.2）。⚠ `notif`（通知偏好）的**内部结构**规格从未定义（只写了 `{...}`）⇒ 本版**未实现该键**（不许由实现反过来定义契约，→《欠账登记表》**D-42**）。**沿用 V1.25**：**补明确**：公海关系上的「**开发价值**」**销售也能标** —— 开发价值是**部门共同维护**的属性（⚠ **不看 owner**：无主关系没有 owner 可看）；**写跟单只能在私海**。**沿用 V1.24**：**公海「可读不可写」口径**（2026-09-18 拍板）：无主（公海）关系 —— ① **读**：详情 ＋ 跟单 ＋ 承诺 ＋ 阶段留痕对**该关系所属部门**可见（销售＝**本部门**、经理＝管辖部门、总经理＝全部；**别部门公海看不到**；交付 / 客服不进公海）；**公司档案层只给基本资料 ＋ 跟单计数（近 30 天 N 次），不给跟单正文**；② **写**：一律拒（**422 / `20408`**，新增码见 §2.4），**唯一例外 ＝ 改「开发价值」`value_tier`**（要跟单**先领取到私海**）。**沿用 V1.23**：**新增「关联公司并激活业务关系」端点**（2026-09-16 拍板 · D-29）：`POST /contacts/:id/activate-relation` —— 把「待关联」联系人关联到公司 ＋ 激活关系 ＋ 把孤儿跟单批量挂过去，**三件事一个动作**（→ §4.4 / §5.6）。**沿用 V1.22**：**联系人列表可见范围 ＋ 待关联归属**（2026-09-16 拍板）：`GET /contacts` 增设 `only_unlinked` 入参（只看"未关联公司"的待跟进）；**「待关联」（未挂公司）联系人只给归属人自己** —— 归属＝**建档录入人**，落新增列 `contact.owner_id`（migration `0007`，→ 需求 §6.1 ⑦~⑪）。⚠ **已挂公司者的"公司维度收敛"待补**：`company`(L2) **不可反向依赖** `relation`(L3)（架构 §3 层级），落地方式待定（→《欠账登记表》D-28）。**沿用 V1.21**：业务关系列表「筛选」落地**：`GET /relations` 增 `view`（`all` 我的全部 / `following` 跟进中＝阶段 1~5 / `cooperated` 已合作＝阶段 6 / `churned` 已流失＝阶段 7）＋ `urgency`（紧迫档**多选**、逗号分隔），→ §4.4 / §5.6。**拍板结论（P-01）＝由后端开参数**、**不放前端本地过滤** —— 分页之后本地过滤只能筛当前页，且「共 N 条」会退化成「本页条数」。⚠ 前端 §5 的第五档 **「逾期未跟进」本版不提供**：逾期唯一来源是承诺 / 预约（D 域），字段尚未落地，**不编一个假的逾期定义**（→《欠账登记表》D-10）。**沿用 V1.20**：业务关系列表分页落地（出参由裸数组改为 §2.3 分页形态 `{list,total,page,page_size}`、入参 `page` / `page_size`）／**§2.6 码表落点收口**（枚举码值不再在本文件列举，一律指向《数据架构文档》§十三 `dict_item` ／ §三~§九 字段口径 —— 消掉「同一串码值有两个落点」这个 **5 次翻车的病根**（→《废止口径登记表》#36 / #37）；阶段 `stage` 数字 `1`~`7` 的**例外提示保留**，仍禁止自造英文码）／承诺三态（`done` / `cancelled` / `waived`）、成功响应一律 200、`20407`＝跨部门 @求助、联系方式默认全可见 ＋ 联系人可上锁、登录＝手机号 / 账号名双通道 · 紧随需求 V1.33 / 数据架构 V1.36 / 前端 V1.19）。
...
- 详情 `{...company字段, completeness:{c1,c2,c3}, profile_tags:{identity:[{tag_id,tag_code,label}],policy:[],decision_chain:{tag_id,label}?}, contacts:[ContactBrief], relations_summary:[{dept,product_line:{id,name,color_key},sign_date,amount|amount_masked}], event_count_30d}`
  ...
  - **⚠ 上条的窗口粒度是合成口径（待拍板）**：规格正文只写「**近 30 天**」，未写「含不含今天」与「按谁的日界」⇒ 现按 **30 个自然日（含今天）＋ UTC 日界** 实现（依据 2026-09-21 拍板 Q1「自然日 30 天」＋《prisma/README.md》§七「UTC 存 / 边界按 UTC」，→ D-54）；**这三条口径未逐字见于规格正文**，已列入《交接说明》§二 待拍板，改口径只需改一处（`engine/domain/event-count-window.ts`）。
...
- `GET /contacts` **入参**（V1.22 起）：`only_unlinked`（`true` ＝只看「**未关联公司**」的待跟进，即"待关联"视图，判定＝该联系人无任何 `company_contact` 记录 → 需求 §6.1 ③）。
  **可见范围**（2026-09-16 定，→ 需求 §6.1 ⑪）：「待关联」（未挂公司）**只给归属人自己**（`contact.owner_id`）；已挂公司者的**公司维度收敛待补**（→《欠账登记表》D-28）。
...
### 5.16 公海 sea（2026-09-20 新增：领取端点形状回填）

> 本节落点是**已实现**的 `POST /sea/company/:id/claim`（片 1 `5c59d9f` / 片 2 `7f24d8f`，真库真机已验）。
> 公海其余端点（`G /sea/company`、`G /sea/department`、`G /sea/records`、`G /sea/manager-todo`、`P /sea/manager-decision`、`G/U /sea/rules`）**尚无形状章节**，随实现分批回填（→《欠账登记表》D-33 ④）。
