...
> 配套真相源：《销售CRM业务需求文档》**V1.28**、《销售CRM数据架构文档》**V1.34**、《销售CRM设计规范》V1.0、《销售CRM前端页面与交互文档》**V1.16**（均 需求规格/）。
> **版本沿革**：文档内不留「修改记录」章节（2026-09-12 决定 →《废止口径登记表》#22），沿革查 `git log --follow -- 需求规格/销售CRM接口API文档.md`。
> 生效日期：2026-09-15 ｜ 状态：**V1.18**（**承诺收尾三态补齐**：`done` 兑现 / `cancelled` 取消（**录错了 / 不成立**，不填原因）/ `waived` 豁免（**确有其事但做不成，必填原因** `waive_reason`）—— 承诺出参加 `waive_reason`、`PUT` 入参形状补全（→ 需求 §10.1 / 数据架构 D1 V1.32）。沿用：阶段 `stage` 为数字 `1`~`7`（英文码作废 →《废止口径登记表》#37）；成功响应一律 200；`20407`＝跨部门 @求助；联系方式默认全可见 ＋ 联系人可上锁；登录＝手机号 / 账号名双通道 · 紧随需求 V1.28 / 数据架构 V1.34 / 前端 V1.16）。
...
- **L0 / L1 的权威定义见 `README.md §八`**。本文只需遵守：**需求一改 → 本文同批改、同批升版**；**同一事实只定义一次** —— 枚举值、错误码、脱敏口径在本文定义后，前端文档只写「见《接口 API 文档》§二」，不复制。
...
- **枚举一律英文码**（示例）：
  - 阶段 `stage`：**数字 `1`~`7`**（1 初步建联 → 2 需求确认 → 3 面访产品讲解 → 4 异议与卡点 → 5 逼单 → 6 已合作（成交 · 终态）→ 7 已流失（终态）），即 **6 个推进阶段 ＋ 1 个流失终态**（`→需求§8.1` `→架构C1`）。
    - ⚠ **2026-09-15 裁定**：原举例的英文码 `first_contact` / `need_confirm` / `demo` / `objection` / `closing` / `cooperated` / `churned` **作废**（见《废止口径登记表》**#37**）。理由：DB 的 `business_relation.stage` / `relation_stage_log.from_stage|to_stage` / `commitment.stage_forward` **全是数值列**，数据架构 §十三 的种子也是 `1`~`7`；**阶段中文名由字典 `workflow_stage` 的 `label` 出**（`→架构A11`），接口不传名。
