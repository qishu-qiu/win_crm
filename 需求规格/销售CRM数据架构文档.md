...
| 项目 | 内容 |
|---|---|
| 版本 / 日期 | **V1.34（现行有效）** / 2026-09-15 |
| 上游 | 《销售CRM业务需求文档》**V1.29**（业务规则唯一来源） |
| 下游 | 《销售CRM接口API文档》**V1.20**、《销售CRM设计规范》**V1.0**、《销售CRM前端页面与交互文档》**V1.16** |
...
| action | 统一编码 `模块.动词`。**★ 已实现清单（与代码同源，只增不改 —— 改名＝历史审计断链）**：`account.login.success` / `account.login.fail` / `account.login.rejected`（A 域登录）· `relation.activate` · `relation.update` · `relation.add_member` · `company.create` · `contact.create` · `event.create` · `commitment.create` · `commitment.update`；**查看类** `event.view`（管理员看跟单全文）· **安全类** `authz.denied`（401 / 403 越权尝试）。**待落（随各自接口）**：`sea.phone.view`（公海看号）· `phone.unlock.*` · `approval.approve` · `company.merge` · `tag.update` · 系统配置 / 字典 / 部门规则修改 |
